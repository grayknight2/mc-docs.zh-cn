...
ms.openlocfilehash: a6e38ae8b79c6f52526a91a48b0a2b96a3d9c285
ms.sourcegitcommit: 9d9795f8a5b50cd5ccc19d3a2773817836446912
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/14/2020
ms.locfileid: "88227951"
---
...
可以使用[适用于 Maven 的 Azure Web 应用插件](/java/api/overview/maven/azure-webapp-maven-plugin/readme)来部署 .war 文件。 [Azure Toolkit for IntelliJ](/java/intellij/azure-toolkit-for-intellij) 或 [Azure Toolkit for Eclipse](/java/eclipse/azure-toolkit-for-eclipse) 还支持通过流行的 IDE 进行部署。
...
使用[符合条件的 Azure 支持计划](https://support.azure.cn/support/plans/)进行 Azure 或 [Azure Stack](https://azure.microsoft.com/overview/azure-stack/) 方面的开发时，可以通过 Microsoft 获得对 [Azure 支持的 Azul Zulu JDK](https://www.azul.com/downloads/azure-only/zulu/) 的产品支持。
...
可以使用[适用于 Azure 应用服务的 Maven 插件](/java/api/overview/maven/azure-webapp-maven-plugin/readme)部署 .jar 和 .war 文件。 [Azure Toolkit for IntelliJ](/java/intellij/azure-toolkit-for-intellij) 或 [Azure Toolkit for Eclipse](/java/eclipse/azure-toolkit-for-eclipse) 也支持使用常用的 IDE 进行部署。

否则，则部署方法将取决于存档类型：

- 若要将 .war 文件部署到 Tomcat，请使用 `/api/wardeploy/` 终结点对存档文件执行 POST 操作。 有关此 API 的详细信息，请参阅[此文档](https://docs.azure.cn/app-service/deploy-zip#deploy-war-file)。
- 若要部署 Java SE 映像中的 .jar 文件，请使用 Kudu 站点的 `/api/zipdeploy/` 终结点。 有关此 API 的详细信息，请参阅[此文档](https://docs.azure.cn/app-service/deploy-zip#rest)。
...
有关详细信息，请参阅[在 Azure CLI 中流式传输日志](troubleshoot-diagnostic-logs.md)。
...
>[!NOTE]
>在 12 小时后禁用记录到本地应用服务文件系统这项功能仅适用于基于 Windows 的应用服务。 只能使用 [Azure Monitor（预览版）](/app-service/troubleshoot-diagnostic-logs#send-logs-to-azure-monitor-preview)配置适用于基于 Linux 的应用服务的 Azure Blob 存储日志记录功能 

<!-- If your application uses [Logback](https://logback.qos.ch/) or [Log4j](https://logging.apache.org/log4j) for tracing, you can forward these traces for review into Azure Application Insights using the logging framework configuration instructions in [Explore Java trace logs in Application Insights](/application-insights/app-insights-java-trace-logs). -->
...
如果要部署 JAR 应用程序，应将其命名为“app.jar”，以便内置映像能正确识别应用。 （Maven 插件自动执行该重命名操作。）如果不希望将 JAR 重命名为 app.jar，可使用命令上传 shell 脚本来运行 JAR。 然后，将此脚本的完整路径粘贴到门户“配置”部分的[启动文件](faq-app-service-linux.md)文本框中。 启动脚本不从放置它的目录运行。 因此，请始终使用绝对路径在启动脚本中引用文件（例如：`java -jar /home/myapp/myapp.jar`）。
...
在适用于 Linux 应用服务中运行的 Java 应用程序实施与其他应用程序相同的一套安全最佳做法。
...
若要注销用户，请使用 `/.auth/ext/logout` 路径。 若要执行其他操作，请参阅介绍[应用服务身份验证和授权的用法](https://docs.azure.cn/app-service/app-service-authentication-how-to)的文档。 还可以阅读关于 Tomcat [HttpServletRequest 接口](https://tomcat.apache.org/tomcat-5.5-doc/servletapi/javax/servlet/http/HttpServletRequest.html)及其方法的官方文档。 以下 Servlet 方法是基于你的应用服务配置解除冻结的：
...
Spring Boot 开发人员可以使用 [Azure Active Directory Spring Boot Starter](/java/spring-framework/configure-spring-boot-starter-java-app-with-azure-active-directory?view=azure-java-stable) 通过熟悉的 Spring Security 注释和 API 来保护应用程序。 务必增加 application.properties 文件中的最大标头大小。 我们建议值为 `16384`。
...
如果要为 JAR 使用不同的名称，还必须提供执行 JAR 文件的[启动命令](faq-app-service-linux.md)。 例如，`java -jar my-jar-app.jar`。 在门户中为启动命令设置值的方式有两种：在“配置”>“常规设置”下设置，或者使用名为 `STARTUP_COMMAND` 的应用程序设置来进行设置。
...
添加共享的服务器级数据源需要编辑 Tomcat 的 server.xml。 首先，上传[启动脚本](faq-app-service-linux.md)，然后在“配置” > “启动命令”中设置该脚本的路径 。 你可以使用 [FTP](deploy-ftp.md) 上传启动脚本。
...
可将 Tomcat 配置为使用外部会话存储，例如 [Azure Cache for Redis](/azure-cache-for-redis/)。 这样，你就能在用户被转到另一个应用实例时（例如，发生自动缩放、重启或故障转移时）保存用户会话状态（例如购物车数据）。
...
3. 使用 FTP 将会话管理器的 JAR 文件上传到应用服务实例，并将它放到 /home/tomcat/lib 目录中。 有关详细信息，请参阅[使用 FTP/S 将应用部署到 Azure 应用服务](https://docs.azure.cn/app-service/deploy-ftp)。
...
8. 更新应用的 pom.xml 文件中的 `azure-webapp-maven-plugin` 配置，以引用 Redis 帐户信息。 该文件使用你之前设置的环境变量，将你的帐户信息保存在源文件以外。
    ...
    如有必要，请将 `1.9.1` 更改为[适用于 Azure 应用服务的 Maven 插件](/java/api/overview/maven/azure-webapp-maven-plugin/readme)的当前版本。
